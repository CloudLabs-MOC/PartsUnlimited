...
1. Go to the link below, login with the details provided to you in environments output page and in you Azure DevOps account click on the **+ Create project** button.
...
11. Select the first **Copy Files to:** task, and fill in the listed fields with the below values, leaving any items not mentioned with their default values.   
 ...
 - **Note**: We will copy the files we want from the build and repo and place them in a staging area on the agent, to be later picked up from there and published as build pipeline artifacts which we can later use in our release pipeline. To view more details about the variables that we are using in the tasks you can take a lok at the page   <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/build/variables?view=vsts" target="_blank"><span style="color: #0066cc;" color="#0066cc">Predefined build variables</span></a> 
